...
void task(void)
{
    ...
    for (i = 0; i < 20; i++) {
        stuff.unum = a * b;
        printf("%x * %x = %x%x\n", (uint32_t) a, (uint32_t) b, stuff.l.high,
               stuff.l.low);
        b *= 3;
    }
    ...
    for (i = 0; i < 20; i++) {
        stuff.unum = a / b;
        printf("%x%x / %x%x = %x%x\n", (uint32_t) (a >> 32),
               (uint32_t) (a & 0xffffffff), (uint32_t) (b >> 32),
               (uint32_t) (b & 0xffffffff), stuff.l.high, stuff.l.low);
        b /= 3;
    }
    ...
    for (i = 0; i < 20; i++) {
        stuff.num = c * d;
        printf("%x * %x = %x%x\n", (uint32_t) c, (uint32_t) d, stuff.l.high,
               stuff.l.low);
        d *= 3;
    }
    ...
    for (i = 0; i < 20; i++) {
        stuff.unum = c / d;
        printf("%x%x / %x%x = %x%x\n", (uint32_t) (c >> 32),
               (uint32_t) (c & 0xffffffff), (uint32_t) (d >> 32),
               (uint32_t) (d & 0xffffffff), stuff.l.high, stuff.l.low);
        d /= 3;
    }
