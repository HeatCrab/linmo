 ...
 *            - Triggers PMP fault and task termination
 *
 *   Test 3: Region Eviction and Lazy Loading
 *           - Validates eviction mechanism when PMP regions are exhausted
 *           - Spawns tasks exceeding hardware region limit
 *           - Verifies lazy loading and context switching correctness
 */
...
/* Eviction test task - spawning multiple instances exceeds PMP region limit
 * and triggers eviction/lazy loading during context switches */
void task_eviction(void)
{
    sys_tdelay(200); /* Wait for Test 2 cleanup */

    int my_tid = sys_tid();
    volatile uint32_t magic = 0xE71C0000 + my_tid;

    /* A few context switches are enough to verify eviction works correctly */
    for (int i = 0; i < 5; i++) {
        sys_tyield();

        if (magic != 0xE71C0000 + my_tid) {
            umode_printf(
                "[Eviction %d] FAIL: Stack corrupted! Expected "
                "0x%08x, got 0x%08x\n",
                my_tid, (unsigned int) (0xE71C0000 + my_tid),
                (unsigned int) magic);
            tests_failed++;
            while (1)
                sys_tyield();
    ...
    umode_printf("[Eviction %d] PASS: Eviction handled correctly\n", my_tid);
    tests_passed++;
...
void monitor_task(void)
{
    ...
    umode_printf("  [Test 3] Region Eviction\n");
    umode_printf("=================================================\n");
    ...
    int test3_passed_before = tests_passed;
    sys_tdelay(150);

    /* Report Test 3 results */
    umode_printf("\n=== Test 3: Region Eviction ===\n");
    int test3_passed = tests_passed - test3_passed_before;
    umode_printf("Eviction tasks passed: %d/3\n", test3_passed);

    if (test3_passed == 3 && tests_failed == 0) {
        umode_printf("Status: PASS\n");
    } else {
        umode_printf("Status: FAIL\n");
    }
    ...
    /* Final summary */
    umode_printf("\n=================================================\n");
    if (tests_failed == 0 && tests_passed >= 6) {
...
int32_t app_main(void)
{
    ...
    /* Test 3: Region Eviction (3 tasks to exceed region limit) */
    int32_t evict_1 = mo_task_spawn_user(task_eviction, 1024);
    int32_t evict_2 = mo_task_spawn_user(task_eviction, 1024);
    int32_t evict_3 = mo_task_spawn_user(task_eviction, 1024);

    if (task_a < 0 || task_b < 0 || task_c < 0 || monitor < 0 ||
        kernel_test < 0 || cross_test < 0 || evict_1 < 0 || evict_2 < 0 ||
        evict_3 < 0) {
